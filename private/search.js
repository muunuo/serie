...
search_.addEventListener('submit', async (event) => {  //async stops search_ from running before the info arrives
    ...
    const searchName_ = input_.value.trim(); //trim removes space at end. Value = gets the user input

    if (!searchName_) return;

    const sendShow_ = await getShowData_(searchName_);

    if (sendShow_ && sendShow_.data) {
        const showId_ = sendShow_.data.id || searchName_.data.tvdb_id;
    ...
    await displayShow_(searchName_) //pauses the function untill displayShow_ runs 
...
async function getShowData_(searchName_) {
    ...
    const res = await fetch(`https://api4.thetvdb.com/v4/search?query=${encodeURIComponent(searchName_)}&type=series`, {
        headers: {
    ...
    if (data.data && data.data.length > 0) {
        const showId_ = data.data[0].tvdb_id;
        const seriesRes_ = await fetch(`https://api4.thetvdb.com/v4/series/${showId_}`, {
            headers: {
                'Authorization': 'Bearer ' + token
            }
        });
