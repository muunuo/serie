...
async function statusSeries_() {
    ...
    for (const series_ of getSeries_) {
            ...
            console.log("showName111" + allData_.name);
...
statusSeries_();
// window.addEventListener('load', statusSeries_);

// async function statusSeries_() {
//     try {
//         const get_ = await fetch(`/api/checkSeriesStaus_`);
//         if (!get_.ok) throw new error ('faild...');
//         const hasSeen_ = await get_.json();

//         const container_ = document.getElementById('statusSeries'); // use div from dashboard.html
//         container_.innerHTML = ""; // Clear previous content

//         if (hasSeen_.length === 0) {
//             container_.innerHTML = "<p>No registered shows yet.</p>";
//             return;
//         }

//         for (const series of hasSeen_) {
...
//     } catch (error) {
//          console.error('Error fetching or displaying shows:', error);
//         const container_ = document.getElementById('statusSeries');
//         container_.innerHTML = "<p>Error loading shows. Please try again.</p>";
//     }
